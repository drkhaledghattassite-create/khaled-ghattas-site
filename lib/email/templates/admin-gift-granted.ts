/**
 * Admin gift granted email — sent to a recipient when Dr. Khaled's team
 * (admin) grants a free gift via /admin/gifts/new. Welcoming tone — recipient
 * may be a brand-new user, so we err on hospitable + clear.
 */
...
export type AdminGiftGrantedEmailInput = {
  locale: GiftEmailLocale
  recipientEmail: string
  item: GiftDisplayItem
  /**
   * If the recipient already had an account (auto-claim), `claimUrl` points
   * at the dashboard surface where the entitlement is now visible. If not
   * (PENDING grant), it points at the /gifts/claim?token=... URL.
   */
  claimUrl: string
...
const L: Record<GiftEmailLocale, Strings> = {
  ar: {
    brand: 'د. خالد غطاس',
    subject: 'فريق د. خالد غطاس أرسل لك هدية',
    receiptLabel: 'هدية لك',
    greeting: 'فريق د. خالد غطاس أرسل لك هدية',
    introPending:
      'يسعدنا أن نُهديك هذا المحتوى. اضغط الزر أدناه لاستلام الهدية.',
    introClaimed:
      'يسعدنا أن نُهديك هذا المحتوى. لقد فتحنا الوصول إليه في حسابك مباشرة.',
    detailsHeading: 'تفاصيل الهدية',
    itemTypeLabel: 'النوع',
    expiresOn: (d) => `تنتهي صلاحية هذه الهدية بتاريخ ${d}.`,
    ctaPending: 'استلم هديتك',
    ctaClaimed: 'افتح هديتك',
    welcomeNote: 'يسرّنا انضمامك إلينا.',
    supportLine: (email) =>
    ...
    messageHeading: 'رسالة من الفريق',
  },
  en: {
    brand: 'Dr. Khaled Ghattass',
    subject: 'A gift from the Dr. Khaled Ghattass team',
    receiptLabel: 'A gift for you',
    greeting: 'The Dr. Khaled Ghattass team sent you a gift',
    introPending:
      'We\'re glad to share this with you. Press the button below to claim it.',
    introClaimed:
      "We're glad to share this with you. We've already added it to your account.",
    detailsHeading: 'Gift details',
    itemTypeLabel: 'Type',
    expiresOn: (d) => `This gift expires on ${d}.`,
    ctaPending: 'Claim your gift',
    ctaClaimed: 'Open your gift',
    welcomeNote: "We're glad to have you here.",
    supportLine: (email) =>
    ...
    messageHeading: 'A note from the team',
  },
}

export function buildAdminGiftGrantedSubject(locale: GiftEmailLocale): string {
  return L[locale].subject
}

export function buildAdminGiftGrantedHtml(
  input: AdminGiftGrantedEmailInput,
): string {
  const { locale, recipientEmail, item, claimUrl, alreadyClaimed, expiresAt, supportEmail } = input
...
<body style="margin:0;padding:0;background:${PALETTE.bgDeep};font-family:${font};color:${PALETTE.fg1};line-height:1.6;">
  <div style="max-width:560px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:${PALETTE.bgElevated};border:1px solid ${PALETTE.border};border-radius:12px;padding:36px 32px;">
      ...
      <h1 style="margin:0 0 8px 0;font-size:24px;line-height:1.3;font-weight:700;color:${PALETTE.fg1};">${t.greeting}</h1>
      <p style="margin:0 0 24px 0;font-size:16px;color:${PALETTE.fg2};">${alreadyClaimed ? t.introClaimed : t.introPending}</p>
      ...
      <div style="text-align:center;margin-bottom:8px;">
        <a href="${claimUrl}" style="display:inline-block;background:${PALETTE.accent};color:${PALETTE.accentFg};padding:14px 32px;border-radius:9999px;font-size:15px;font-weight:600;text-decoration:none;">${alreadyClaimed ? t.ctaClaimed : t.ctaPending}</a>
      </div>
      <p style="margin:14px 0 0 0;font-size:13px;color:${PALETTE.fg2};text-align:center;">${t.welcomeNote}</p>
      ${alreadyClaimed ? '' : `<p style="margin:8px 0 0 0;font-size:13px;color:${PALETTE.fg3};text-align:center;">${t.expiresOn(fmtDate(expiresAt, locale))}</p>`}
    </div>
...
export function buildAdminGiftGrantedText(
  input: AdminGiftGrantedEmailInput,
): string {
  const { locale, recipientEmail, item, claimUrl, alreadyClaimed, expiresAt, supportEmail } = input
  const t = L[locale]
  ...
  const lines = [
    ...
    t.greeting,
    '',
    alreadyClaimed ? t.introClaimed : t.introPending,
    '',
    `${t.detailsHeading}:`,
    `  ${title}`,
    ...
    message ? '' : null,
    `${alreadyClaimed ? t.ctaClaimed : t.ctaPending}: ${claimUrl}`,
    '',
    t.welcomeNote,
    alreadyClaimed ? null : t.expiresOn(fmtDate(expiresAt, locale)),
    '',
    `${supportEmail}`,
    t.copyright,
