...
/**
 * Section .05 — Interviews. A sticky scroll moment where five clippings
 * fly in over a warm rose-paper disc, then the camera tightens onto a
 * brass stamp CTA. Reduced from the original to a quieter, journal-room
 * choreography.
 */
export function InterviewRotator({ interviews }: { interviews: Interview[] }) {
  ...
  // Background warms from cream → skin tone as the cards land
  const bg = useTransform(
    scrollYProgress,
    [0, 0.25, 1],
    ['#F0E6D8', '#EBDDC9', '#E4D0B7'],
  )
  // Use a fixed maximum size, then scale (composite-only) instead of
  // animating width (layout). 80vmin caps to viewport on mobile.
